...
void CPlayCamera::Step(VECTOR focus, float rot)
{
	...
	VECTOR	playerPos = focus;
	playerPos.y += 15.0f;	// ちょっと上にあげる
	MATRIX	mMat1, mMat2;
	// 回転行列を利用してカメラ位置設定
	mMat1 = MGetTranslate(camDist);
	mMat2 = MGetRotX(m_camRot.x);
	mMat1 = MMult(mMat1, mMat2);
	mMat2 = MGetRotY(m_camRot.y);
	mMat1 = MMult(mMat1, mMat2);
	camDist = VGet(mMat1.m[3][0], mMat1.m[3][1], mMat1.m[3][2]);
...
#else
	static const float MAXLEN = 50.0f;		// これより距離が離れると移動開始
	static const float MAXLEN2 = 60.0f;		// これ以上は離れさせない
	static const float MINLEN = 40.0f;		// これより距離が近づくと移動開始
	static const float MINLEN2 = 30.0f;		// これ以上は近づけさせない
