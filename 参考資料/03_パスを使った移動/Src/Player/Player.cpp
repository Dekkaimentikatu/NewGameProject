...
//		コンストラクタ
//------------------------------
Player::Player() : m_camRot({0.0f}), m_gravity(0.0f)
{
}
...
void	Player::ExecWalk()
{
	...
	// 通常攻撃
	if (CInputKey::IsPushKeyTrg(KEY_INPUT_Z))
	{
		Model::RequestEndLoop(PLSTATE_ATTACK1, ANIM_SPD);
	}
	// しゃがみ
	else if(CInputKey::IsPushKeyLaw(KEY_INPUT_X))
	{
	...
	else
	{
		...
		{
			// シフトキーを押していたら走りへ
			if (CInputKey::IsPushKeyLaw(KEY_INPUT_LSHIFT))
			{
...
{
	// しゃがみ
	if (CInputKey::IsPushKeyLaw(KEY_INPUT_X))
	{
		// しゃがみ中はしゃがみ攻撃に派生する
		if (CInputKey::IsPushKeyTrg(KEY_INPUT_Z))
		{
...
VECTOR	Player::GetSpeed()
{
	...
	if (CInputKey::IsPushKeyLaw(KEY_INPUT_D))
	{
		plSpd.x -= spd;
	}
	if (CInputKey::IsPushKeyLaw(KEY_INPUT_A))
	{
		plSpd.x += spd;
	}
	if (CInputKey::IsPushKeyLaw(KEY_INPUT_W))
	{
		plSpd.z -= spd;
	}
	if (CInputKey::IsPushKeyLaw(KEY_INPUT_S))
	{
		plSpd.z += spd;
	}
