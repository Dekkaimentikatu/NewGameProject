...
//		初期化
//-------------------------------
void CDebugCamera::Init(VECTOR pos, VECTOR focus, VECTOR up)
{
	m_pos = pos;
	m_focus = focus;
	m_up = up;
...
{
	VECTOR vVec1, vVec2, vCross;
	// 前進
	if (CheckHitKey(KEY_INPUT_A))
	{
		// 視点・注視点から2つの方向ベクトルを取得
 		vVec1 = VSub(m_pos, m_focus);
		vVec2 = vVec1;
		vVec2.y -= 1.0f;
		// 外積を求めて移動速度に合わせる
		vCross = VCross(vVec1, vVec2);
		vCross = VNorm(vCross);
		vCross = VScale(vCross, CAMERA_SPEED);
		// 現在の視点･注視点に加算
		m_pos = VAdd(m_pos, vCross);
		m_focus = VAdd(m_focus, vCross);
	}
...
void CDebugCamera::RotateLeftRight()
{
	...
	if (fDir != 0.0f)
	{
		...
		VECTOR vec = VSub(m_focus, m_pos);
		vec.y = 0.0f;
		float rot = atan2f(vec.z, vec.x);

		// 求めた回転角度に+1or-1度回転させる
		rot += DEG_TO_RAD(1.0f) * fDir;

		// 元々の視点から注視点までの距離を使い、ベクトルを回転
		float fLength = VSize(vec);
		vec.x = cosf(rot) * fLength;
		vec.z = sinf(rot) * fLength;
		// 注視点の位置を視点+求めたベクトルにする
		m_focus.x = m_pos.x + vec.x;
		m_focus.z = m_pos.z + vec.z;
	}
}
